  ...
  if ((id_account, code)) {
    ...
    if (checkCode.length > 0) {
      ...
      if (checkIdAccount.length > 0) {
        const checkHistoryReedem = await historyReedemModel.checkHistoryReedem(
          id_account,
          code
        );
        if (checkHistoryReedem.length > 0) {
          ...
          const plusSaldoUser =
            parseInt(checkIdAccount[0].saldo) + parseInt(checkCode[0].reward);
          const processReedem = await transferModel.updateSaldo(
            id_account,
            plusSaldoUser
          );
          if (processReedem) {
            await historyReedemModel.createHistory(
              checkIdAccount[0].name,
              id_account,
              code
            );
            res.json({
