...
exports.createAccount = async (req, res) => {
  ...
  if ((name, email, password, pin, id_account)) {
    ...
    if (check.length > 0) {
      ...
      const createAccount = await accountModel.createAccount(
        name,
        email,
        password,
        pin,
  ...
  if ((bank_name, name_card_holder, id_account, card_number, phone, address)) {
    ...
    if (checkStatusAccount.length > 0) {
      if (checkStatusAccount[0].status == 1) {
        ...
        const activatedAccount = await accountModel.activatedAccount(
          bank_name,
          name_card_holder,
          id_account,
          card_number,
        ...
        const changeStatusAccount = await accountModel.changeStatusAccount(
          id_account,
          "1"
        );
        if (activatedAccount && checkStatusAccount) {
          await historyModel.createHistory(
            checkStatusAccount[0].name,
            id_account,
            "Activated account"
          );
          res.json({
