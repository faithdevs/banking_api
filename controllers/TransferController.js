    ...
    if (nominal < 5000) {
      ...
      if (checkFrom.length > 0) {
        if (checkFrom[0].saldo < 5000) {
          ...
          if (checkTo.length > 0) {
            const minusSaldoFrom =
              parseInt(checkFrom[0].saldo) - parseInt(nominal);
            ...
            await historyModel.createHistory(
              checkFrom[0].name,
              checkFrom[0].id_account,
              "Transfer saldo to " + to_id_account + " nominal " + nominal
            );
            await historyModel.createHistory(
              checkTo[0].name,
              checkTo[0].id_account,
              "Receive saldo from " + from_id_account + " nominal " + nominal
            );
            res.json({
              status: true,
